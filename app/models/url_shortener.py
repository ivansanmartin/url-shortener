...
class UrlShortener(BaseModel):
    _id: str = Field(..., description="Unique ID for the MongoDB document")
    original_url: HttpUrl = Field(..., description="The original URL to be shortened")
    short_url: HttpUrl = Field(..., description="The generated shortened URL")
    slug: str = Field(..., description="Unique identifier for the shortened URL")
    created_at: Optional[datetime] = Field(default_factory=datetime.datetime.now(datetime.timezone.utc), description="Date and time the URL was created")
